// import the cds facade object (https://cap.cloud.sap/docs/nodejs/cdsfacade)
const cds = require('@sap/cds')
// the service implementation with all service handlers
module.exports = cds.service.impl(async function() {
    // define constants for the Risks and BusinessPartner entities from the risk-service.cds file
    const {Risks, BusinessPartners} = this.entities
    // This handler will be executed immediately AFTER a READ on RISKS
    // With this we can loop through the entire data set and manipulate each risk entries
    this.after('READ', 'Risks', (data) => {
        //convert to array if it's only a single risk so that code won't break here.
        const risks = Array.isArray(data) ? data : [data]
        //looping through the array of risks to set the virtual field criticality tha you defined in the schema
        risks.forEach(risk => {
